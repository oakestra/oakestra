...
wget -c https://github.com/oakestra/oakestra/releases/download/$OAKESTRA_VERSION/NodeEngine_$(dpkg --print-architecture).tar.gz && tar -xzf NodeEngine_$(dpkg --print-architecture).tar.gz && chmod +x install.sh && ./install.sh $(dpkg --print-architecture)
if [ $? -ne 0 ]; then
