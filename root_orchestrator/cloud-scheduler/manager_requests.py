...
SYSTEM_MANAGER_ADDR = 'http://' + os.environ.get('SYSTEM_MANAGER_URL') + ':' + str(os.environ.get('SYSTEM_MANAGER_PORT'))
...
def manager_request_replicate(cluster, job_id, job, replicas):
    ...
    try:
        requests.post(request_address, json=dumps({'cluster': cluster, 'job': job, 'job_id': job_id, 'replicas': replicas}))
    except requests.exceptions.RequestException as e:
