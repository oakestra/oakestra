...
def mongo_insert_job(obj):
    ...
    # job insertion
    new_job = mongo_services.find_one_and_update(
        {'job_name': job_name},
        {'$set': job_content},
        upsert=True,
...
def mongo_get_all_jobs():
    return mongo_services.find()


def mongo_get_job_status(job_id):
    return mongo_services.find_one({'_id': ObjectId(job_id)}, {'status': 1})['status'] + '\n'


def mongo_update_job_status(job_id, status, instances=None):
    job = mongo_services.find_one({'_id': ObjectId(job_id)})
    ...
    return mongo_services.update_one(
        {'_id': ObjectId(job_id)},
...
def mongo_set_microservice_id(job_id):
    return mongo_services.update_one({'_id': ObjectId(job_id)}, {'$set': {'microserviceID': job_id}})
...
def mongo_update_job_net_status(job_id, instances):
    ...
    for instance in instances:
        ...
        instance_list[instance_num] = elem
    mongo_services.update_one({'_id': ObjectId(job_id)}, {'$set': {'instance_list': instance_list}})


def mongo_find_job_by_id(job_id):
    return mongo_services.find_one(ObjectId(job_id))


def mongo_find_job_by_name(job_name):
    return mongo_services.find_one({'job_name': job_name})


def mongo_find_job_by_ip(ip):
    # Search by Service Ip
    job = mongo_services.find_one({'service_ip_list.Address': ip})
    if job is None:
        # Search by instance ip
        job = mongo_services.find_one({'instance_list.instance_ip': ip})
    return job
...
def mongo_update_job_status_and_instances(job_id, status, replicas, instance_list):
    ...
    mongo_services.update_one({'_id': ObjectId(job_id)},
                              {'$set': {'status': status, 'replicas': replicas, 'instance_list': instance_list}})


def mongo_get_jobs_of_application(app_id):
    return mongo_services.aggregate([{'$match': {'applicationID': app_id}}])


def mongo_update_job(job_id, job):
    app.logger.info("MONGODB - update job...")
    job = mongo_services.find_one_and_update({'_id': ObjectId(job_id)},
                                             {'$set': job}, return_document=True)
    app.logger.info("MONGODB - job {} updated")
    return job


def mongo_delete_job(job_id):
    global mongo_jobs
    app.logger.info("MONGODB - delete job...")
    mongo_services.find_one_and_delete({'_id': ObjectId(job_id)})
    app.logger.info("MONGODB - job {} deleted")
    # return mongo_frontend_jobs.find()


def mongo_get_job_usage(job_id):
    global mongo_jobs
    app.logger.info("MONGODB - get usage...")
    job = mongo_services.find_one(ObjectId(job_id))
    if "usage" in job:
        return job['usage']
    else:
        return None
...
def mongo_find_cluster_of_job(job_id):
    ...
    job_obj = mongo_services.find_one({'_id': ObjectId(job_id)},
...
def mongo_add_application(application):
    app.logger.info("MONGODB - insert application...")
    user = application.get('userId')
    new_job = mongo_applications.insert_one(application)
    inserted_id = new_job.inserted_id
    app.logger.info("MONGODB - app {} inserted".format(str(inserted_id)))
    mongo_applications.find_one_and_update({'_id': inserted_id},
...
def mongo_get_all_applications():
    return mongo_applications.find()


def mongo_find_app_by_id(app_id, userid):
    return mongo_applications.find_one({'_id': ObjectId(app_id), 'userId': userid})


def mongo_update_application(app_id, userid, data):
    app.logger.info("MONGODB - update data...")
    mongo_applications.find_one_and_update({'_id': ObjectId(app_id), 'userId': userid},
                                           {'$set': {'name': data.get('name'),
                                                     'description': data.get('description'),
                                                     'namespace': data.get('namespace')}})

    app.logger.info("MONGODB - application {} updated")
    return mongo_applications.find()  # return the application list


def mongo_update_application_microservices(app_id, microservices):
    mongo_applications.find_one_and_update({'_id': ObjectId(app_id)},
                                           {'$set': {'microservices': microservices}})


def mongo_delete_application(app_id, userid):
    mongo_applications.find_one_and_delete({'_id': ObjectId(app_id), 'userId': userid})
    return mongo_applications.find()  # return the application list


def mongo_get_applications_of_user(user_id):
    return mongo_applications.aggregate([{'$match': {"userId": user_id}}])
