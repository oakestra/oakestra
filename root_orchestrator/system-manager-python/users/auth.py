        ...
        if user_struct is not None:
            ...
            if check_password_hash(user_struct.get('password'), password):
                access_token = securityUtils.create_jwt_auth_access_token(
                    identity=username,
                    additional_claims={
                        'user': username, 'roles': roles, 'organization': organization_id}
                )
                ...
                logging.log(
                    level=logging.ERROR, msg="Invalid password provided from user: " + username)
        else:
...
def user_token_refresh(username):
    ...
    if user is None:
        return {}
    return {
        "token": securityUtils.create_jwt_auth_access_token(identity=username, additional_claims={"user": username, 'roles': claims['roles'], 'organization': claims['organization']})
    }
...
def user_create_password_reset_request(username, domain, reset_token, expiration):
    ...
    reset_token_hash = hashlib.pbkdf2_hmac(
        'sha256', reset_token.encode('ascii'), b'', 100000).hex()
