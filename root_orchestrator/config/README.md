...
The first LogQL path requires `schema_version=1` and the Alloy-extracted `level` label to be `error` or `critical`. This is the authoritative path for Oakestra Python, standard-library, and Gunicorn records: an INFO message merely containing the word `ERROR` does not fire. A separate compatibility path excludes schema-v1 records before matching uppercase `ERROR`, `FATAL`, `CRITICAL`, and `EXCEPTION`; Oakestra compact `[E...]` and `[F...]` records; Python traceback markers written directly to stderr; and Go panic/stack markers. Grafana, Loki, Alloy, and Prometheus streams are excluded to prevent recursive or infrastructure-generated alert noise.
