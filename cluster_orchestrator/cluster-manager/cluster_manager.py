...
from cluster_scheduler_requests import (
    scheduler_request_deploy,
    scheduler_request_replicate,
...
def get_scheduler_result_and_propagate_to_edge(system_job_id, instance_number):
    ...
    if data.get("found", False):
        ...
        mongo_update_job_status(
            system_job_id, instance_number, "NODE_SCHEDULED", data.get("node")
        )
        ...
        mongo_update_job_status(
            system_job_id, instance_number, "NO_WORKER_CAPACITY", None
        )
    return "ok"
...
def handle_init_worker(message):
    ...
    client_id = mongo_upsert_node(
        {"ip": request.remote_addr, "node_info": json.loads(message)}
    )
...
def handle_init_greeting(jsonarg):
    app.logger.info(
        "Websocket - received System_Manager_to_Cluster_Manager_1 : " + str(jsonarg)
    )
    data = {
    ...
    app.logger.info(
        "Websocket - Cluster Info sent. (Cluster_Manager_to_System_Manager)"
    )
...
def handle_init_final(jsonarg):
    app.logger.info(
        "Websocket - received System_Manager_to_Cluster_Manager_2:" + str(jsonarg)
    )
...
def init_cm_to_sm():
    ...
    try:
        ...
        app.logger.error(
            "SocketIO - Connection Establishment with System Manager failed!"
        )
    time.sleep(1)
...
def background_job_send_aggregated_information_to_sm():
    ...
    job_re_deploy_dead_jobs = scheduler.add_job(
        re_deploy_dead_services_routine, "interval", seconds=BACKGROUND_JOB_INTERVAL
    )
