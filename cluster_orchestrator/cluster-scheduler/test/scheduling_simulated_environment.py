...
# TODO: Adapt these values to change:
...
number_of_nodes = 100
# Dimension of the Vivaldi Network Coordinates
...
# Target worker node for Service-to-Service scheduling
s2s_target_id = "6"
...
class SchedulingSimulatedEnvironment(unittest.TestCase):
    ...
    def test_benchmark_sla_alarm_with_s2u_s2s_constraints_single_viv(self):
        ...
        measurements, user_measurements, worker_list, worker_ip_map, vivs = prepare(session=session, nr_nodes=number_of_nodes, iterations=iterations,
                                                                                    neighbors=neighbors,
    ...
    def print_calculation_csv(self):
        ...
        import os
        x = os.getcwd()
        with open(f"results/casa_{number_of_nodes}workers.csv", "w", newline="") as f:
        ...
        with open(f"results/nat_{number_of_nodes}workers_.csv", "w", newline="") as f:
            csv_writer = csv.writer(f, delimiter=",")
...
# Utility functions to create the simulated edge environment
