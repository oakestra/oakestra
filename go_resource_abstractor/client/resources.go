...
// ResourceFilter narrows ResourcesClient.List. The functions below are the
...
// window (30 seconds).
//
// It takes no argument on purpose. The service applies the freshness filter
// only when ?active= is truthy (BuildCandidateFilter in
// go_resource_abstractor/db/candidates.go), so `active=false` is not "only
// stale candidates" - it is no filter at all, which is what leaving Active
// off already says. An Active(bool) would read as an inverse the service
// doesn't implement.
...
// Fields extends the canonical projection with further document fields, for
// reading data the service stores but doesn't return by default:
//
//	c.Resources.List(ctx, client.Fields("gpu_temp", "gpu_drivers"))
//
// The spec sends them as one comma-separated value; pass them as separate
// arguments and let the generated encoder join them.
func Fields(names ...string) ResourceFilter {
...
func (r *ResourcesClient) List(ctx context.Context, filters ...ResourceFilter) ([]Resource, error) {
...
func (r *ResourcesClient) GetByID(ctx context.Context, id string) (*Resource, error) {
...
func (r *ResourcesClient) GetByName(ctx context.Context, name string) (*Resource, error) {
...
func (r *ResourcesClient) GetByIP(ctx context.Context, ip string) (*Resource, error) {
...
func (r *ResourcesClient) UpdateInformation(ctx context.Context, id string, resource Resource) (*Resource, error) {
...
// a PUT, not a POST.
func (r *ResourcesClient) Create(ctx context.Context, resource Resource) (*Resource, error) {
