...
// TestTrailingSlashesResolveIdentically covers stripTrailingSlash for item
// routes. Collection routes are already covered incidentally, since the
// rest of the suite addresses them with a trailing slash throughout.
...
// An absent filter and an empty one ("?candidate_name=") both mean "no
// filter". Binding the empty pointer straight into the query would instead
// match only documents whose field is literally "".
...
// (model.AsyncEvents / model.SyncEvents) in step with the HookEvent enum the
// API validates against. A name in one list but not the other would let a
// hook register for an event that never fires, or fire one no client can
// subscribe to.
func TestHookEventsMatchSpecEnum(t *testing.T) {
	dispatched := append(append([]model.HookEvent{}, model.AsyncEvents...), model.SyncEvents...)
...
// write decodes into the types generated from openapi.yaml, catching e.g.
...
func TestResponsesMatchGeneratedModels(t *testing.T) {
	...
	t.Run("job", func(t *testing.T) {
		...
		// PATCH populates cpu_percent/memory_percent and their history.
		...
		// match, not a bare JobInstance, so decoding into openapi.Job here
		// is itself the check.
...
// A list endpoint with no matches must answer [] rather than null, since
// consumers iterate the body directly and a nil Go slice would otherwise
// marshal as null.
func TestEmptyListsEncodeAsArray(t *testing.T) {
	resourceType := uniqueName("empty-list")
	registerCustomResourceType(t, resourceType)
	miss := uniqueName("no-match")

	paths := []string{
		"/api/v1/applications/?application_name=" + miss,
		"/api/v1/jobs/?job_name=" + miss,
		"/api/v1/resources/?candidate_name=" + miss,
		"/api/v1/custom-resources/" + resourceType,
	}
