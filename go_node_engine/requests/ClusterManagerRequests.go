...
// clusterClient returns the HTTP client and scheme for calls to the cluster gateway.
// In gateway setups the client presents the worker certificate.
func clusterClient(cfg config.ConfFile) (*http.Client, string, error) {
	if !cfg.ClusterSSL {
	...
	if !gatewaySetup || cfg.WorkerCertFile == "" || cfg.WorkerKeyFile == "" {
		return http.DefaultClient, "https", nil
	}
	cert, err := tls.LoadX509KeyPair(cfg.WorkerCertFile, cfg.WorkerKeyFile)
	if err != nil {
		return nil, "", fmt.Errorf("failed to load worker cert: %v", err)
	}
	...
	default:
		...
		if trustFile != "" {
			...
			if err != nil {
				return nil, "", fmt.Errorf("failed to read gateway trust file %s: %v", trustFile, err)
			}
			pool := x509.NewCertPool()
			if !pool.AppendCertsFromPEM(caPem) {
				return nil, "", fmt.Errorf("failed to parse gateway trust PEM %s", trustFile)
			}
