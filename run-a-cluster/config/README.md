...
node_exporter measures the local physical host, so alert instances carry `host_scope` and `instance`; filesystem alerts also carry `device` and `mountpoint`. They deliberately do not invent a `cluster_id` for host-wide data. A 1-DOC deployment reports one `host_scope="one-doc"` host shared by its Root and Cluster services.
