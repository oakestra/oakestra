...
- `libraries/oakestra_logging` — Shared structured logging configuration and schema for Python services.
...
- **Local dev / tests** — install the required packages directly from `libraries/` (the VS Code `install-*-dependencies` tasks already do this). Service `requirements.txt` files no longer list internal libraries.
...
- **Metrics mounts are sensitive.** cAdvisor receives read-only host, Docker socket, and containerd socket mounts. The containerd socket is required when Docker uses its containerd snapshotter. Read-only socket access still exposes privileged daemon metadata, so cAdvisor's diagnostic UI is loopback-only; Prometheus and node_exporter remain internal in normal deployments.
