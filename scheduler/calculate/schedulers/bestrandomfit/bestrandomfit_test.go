...
func res(id string, mem, cpu, cpuPct float64) Resources {
	return Resources{BaseResources: placement.BaseResources{
		ID:             id,
...
func TestSchedulerNoCandidates(t *testing.T) {
	...
	if !errors.As(err, &placement.SchedulingError{}) {
		t.Fatalf("expected scheduling error, got %v", err)
	}
}

func TestSchedulerFiltersUnqualified(t *testing.T) {
	var a Scheduler
	job := res("j", 1000, 4, 0)
	// Only c2 has enough cpu+mem.
	c1 := res("c1", 500, 1, 0)
	c2 := res("c2", 2000, 8, 0)
	got, err := a.Calculate(job, []Resources{c1, c2})
	if err != nil {
...
func TestSchedulerPicksAmongBestBand(t *testing.T) {
	job := res("j", 100, 1, 0)
	...
	cands := []Resources{worse, top1, top2}

	seen := map[string]int{}
	for i := 0; i < 200; i++ {
		a := Scheduler{Tolerance: DefaultTolerance}
...
func TestSchedulerBandRespectsTolerance(t *testing.T) {
	job := res("j", 100, 1, 0)
	...
	var bandSize int
	a := Scheduler{Tolerance: 2.0, rng: func(n int) int { bandSize = n; return n - 1 }}
	_, err := a.Calculate(job, []Resources{far, near, best})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if bandSize != 2 {
